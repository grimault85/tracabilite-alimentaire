...
import {
  ...
  ajouterLignes, supprimerLivraison, candidatsParGtin, lignesRecentes,
} from "./stockage.js";
import {
  lireEtiquette, lireBonLivraison, compresser, compresserBl,
  traiterAttente, configuree,
} from "./extraction.js";
...
/* ------------------- Choix manuel d'une ligne de BL -------------------
   Indispensable : la plupart des fournisseurs n'impriment pas de GTIN,
   le rapprochement automatique est alors hors de portée.
   --------------------------------------------------------------------- */

function ChoixLigne({ onChoisir, onFermer }) {
  const [lignes, setLignes] = useState(null);
  const [q, setQ] = useState("");

  useEffect(() => { lignesRecentes(30).then(setLignes); }, []);

  const filtrees = useMemo(() => {
    if (!lignes) return [];
    const t = q.trim().toLowerCase();
    if (!t) return lignes.slice(0, 60);
    return lignes.filter(({ ligne, bl }) =>
      [ligne.designation, ligne.marque, ligne.codeArticle, bl.fournisseur]
    ...
    <div className="voile" onClick={onFermer}>
      <div className="panneau" onClick={(ev)=>ev.stopPropagation()}>
        <div className="panneau-corps">
          <p className="label">Rattacher à une livraison</p>
          <h2 className="panneau-titre">Choisir la ligne</h2>
          <input className="input" value={q} onChange={(e)=>setQ(e.target.value)}
            placeholder="Produit, marque ou fournisseur" autoFocus />

          {lignes === null && <p className="vide-texte" style={{ marginTop:16 }}>Chargement…</p>}

          {lignes !== null && lignes.length === 0 && (
            <div className="vide">
              <p className="vide-titre">Aucune livraison enregistrée</p>
              <p className="vide-texte">
                Scanne un bon de livraison depuis l'onglet Livraisons,
                puis reviens rattacher cette étiquette.
              </p>
            </div>
          )}

          {filtrees.map(({ ligne, bl }) => (
            <button key={ligne.id} className="match-choix"
              onClick={()=>{ onChoisir({ ligne, bl }); onFermer(); }}>
              <span className="match-nom">{ligne.designation}</span>
              <span className="match-meta">
                {bl.fournisseur} · {dateCourte(bl.dateReception)}
                {ligne.marque ? ` · ${ligne.marque}` : ""}
                {ligne.dlc ? ` · DLC ${dateFR(ligne.dlc)}` : ""}
              </span>
            </button>
          ))}

          <button className="btn btn-fant" style={{ marginTop:16 }} onClick={onFermer}>
            Annuler
          </button>
        </div>
      </div>
...
function VueScanner({ onEnregistre }) {
  ...
  const analyser = useCallback(async (fichier) => {
    ...
    setEtape("analyse");
    try {
      const { blob: b, base64 } = await compresser(fichier);
      setApercu((ancien) => { if (ancien) URL.revokeObjectURL(ancien); return URL.createObjectURL(b); });
      setBlob(b);
      try {
  ...
  async function enregistrer() {
    const donnees = {
      ...
      rattachement: rattache ? (form.gtin && rattache.ligne.gtin === form.gtin ? "gtin" : "manuel") : "aucun",
  ...
  function recommencer() {
    ...
    setCandidats([]); setRattache(null); setChoixOuvert(false);
      ...
      {etape === "verification" && (
        <div>
          ...
          {rattache ? (
            ...
            <button className="plier" style={{ textAlign:"center", marginTop:0 }}
              onClick={()=>setChoixOuvert(true)}>
              Rattacher à une livraison
            </button>
          )}
          ...
          {choixOuvert && (
            <ChoixLigne
              ...
              onChoisir={(c)=>{
                setRattache(c);
                // Le BL porte parfois la DLC et le lot : on les reprend
                // seulement si l'étiquette n'a rien donné.
                setForm((f)=>({
                  ...f,
                  produit: f.produit || c.ligne.designation || "",
                  marque:  f.marque  || c.ligne.marque || "",
                  lot:     f.lot     || c.ligne.lot || "",
                  dlc:     f.dlc     || c.ligne.dlc || "",
                }));
              }} />
          )}
...
function VueLivraisons({ livraisons, etiquettes, onChange }) {
  ...
  async function analyser(fichier, blExistant = null) {
    ...
    try {
      const { blob: b, base64 } = await compresserBl(fichier);
      setApercu((ancien) => { if (ancien) URL.revokeObjectURL(ancien); return URL.createObjectURL(b); });
      setBlob(b);
      ...
      <div className="page">
        ...
        {!cibleAjout && (
          <div className="entete-bl">
            ...
            {lu.typeDocument && lu.typeDocument !== "bon_livraison" && (
              <div className="entete-type">
                {{ facture:"Facture de livraison", devis:"Devis" }[lu.typeDocument]}
              </div>
            )}
        ...
        {sansGtin > 0 && (
          <div className="avis avis-attention">
            {sansGtin} ligne{sansGtin>1?"s":""} sans code GTIN. La plupart des
            fournisseurs n'en impriment pas : le rattachement se fera à la main
            au moment du scan, depuis « Rattacher à une livraison ».
          </div>
        )}
          ...
          <div className="ligne" key={i}>
            ...
            <div className="ligne-corps">
              ...
              <div className="ligne-meta">
                ...
                {l.lot ? <> · Lot <span className="mono">{l.lot}</span></> : null}
                {l.dlc ? <> · DLC <span className="mono">{dateFR(l.dlc)}</span></> : null}
              </div>
            </div>
          </div>
        ))}
    ...
    return (
      <div>
        <div className="page" style={{ paddingBottom:10 }}>
          ...
          <ChampPhoto icone="▥" titre="Ajouter une partie"
            aide="Deuxième page, ou bas du tableau pris de plus près"
    ...
    <div>
      <div className="page">
        ...
        <ChampPhoto icone="▥" titre="Scanner un bon de livraison"
          aide="Cadre le tableau au plus près, sans les marges de la feuille"
      ...
      <div className="panneau" onClick={(ev)=>ev.stopPropagation()}>
        ...
        <div className="panneau-corps">
          ...
          {[
            ...
            ["Rattachement", { gtin:"Par code GTIN", manuel:"Choisi à la main", aucun:"Aucun" }[e.rattachement] || "Aucun", false],
          ].map(([k,v,m]) => (
...
export default function App() {
  ...
  /* Sans console sur mobile, une erreur non rattrapée laisse l'utilisateur
     devant un écran figé sans explication. On l'affiche. */
  useEffect(() => {
    const surErreur = (ev) => {
      const m = ev.error?.message || ev.message || "Erreur inattendue.";
      setErreur(`Anomalie : ${m}`);
    };
    const surRejet = (ev) => {
